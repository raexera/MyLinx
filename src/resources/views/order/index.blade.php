<x-app-layout>
        ...
        >
            <div class="flex flex-col">
                ...
                <p class="text-[14px] sm:text-[14.5px] font-medium text-[#6A7B8C] leading-snug">
                    ...
                    @if ($thisWeekCount > 0)
                        Minggu ini:
                        <span class="text-[#1A1C19] font-bold"
                            >{{ $thisWeekCount }} pesanan</span
                        >
                        @if ($weekDelta !== 0)
                            <span
                                class="{{ $weekDelta > 0 ? 'text-[#2E5136]' : 'text-red-500' }} font-bold"
                            >
            ...
            <a
                href="{{ route('order.export', request()->query()) }}"
    ...
    @if (session('success'))
        <div
            class="mt-4 rounded-2xl bg-[#ECFDF5] border border-[#A7F3D0] px-5 py-4 text-[13.5px] font-semibold text-[#065F46] flex items-center gap-3"
        >
    ...
    <div class="w-full lg:pr-4 xl:pr-8 pb-12 flex flex-col h-full mt-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
            <div
                class="bg-white rounded-[1.5rem] p-6 border border-[#E8EBED] shadow-[0_2px_10px_rgb(0,0,0,0.015)] relative overflow-hidden group"
            >
                <div
                    class="absolute -right-6 -top-6 text-[#f9fafb] group-hover:scale-110 transition-transform"
                >
                ...
                <div class="relative z-10 flex flex-col h-full">
                    <h3
                        class="text-[11.5px] font-bold text-gray-400 tracking-widest uppercase mb-4"
                    >
                        TOTAL ORDERS
                    </h3>
                    <div class="flex items-end gap-3 mt-auto">
                        <span
                            class="text-[2.5rem] font-serif text-[#1A1C19] leading-none mb-0.5"
                            >{{ $stats['total'] }}</span
                        >
                    </div>
                </div>
            ...
            >
                ...
                <div class="relative z-10 flex flex-col h-full">
                    ...
                    <div class="flex items-end gap-3 mt-auto">
                        ...
                        @if ($stats['pending'] > 0)
                            <span
                                class="text-[#f59e0b] text-[10.5px] font-bold mb-1"
                                >Action needed</span
                            >
                        @endif
                    </div>
        ...
        >
            ...
            />
            <div
                class="flex items-center bg-[#f9fafb] border border-[#E8EBED] rounded-full p-[5px] shadow-[inset_0_1px_2px_rgb(0,0,0,0.01)] overflow-x-auto hide-scroll shrink-0"
            >
        ...
        @if (request('search') || request('status'))
            <div class="flex items-center gap-2 mb-4 px-1">
                ...
                @if (request('status'))
                    <span
                        class="inline-flex items-center gap-1.5 bg-[#EAF2ED] text-[#2E5136] text-[11px] font-bold px-3 py-1 rounded-full"
                    >
                ...
                @if (request('search'))
                    <span
                        class="inline-flex items-center gap-1.5 bg-gray-100 text-gray-700 text-[11px] font-bold px-3 py-1 rounded-full"
                    >
                        "{{ request('search') }}"
                    </span>
                @endif
                <a
                    ...
                    class="text-[11px] font-bold text-gray-400 hover:text-red-500 underline underline-offset-2"
                >
                    Hapus semua
                </a>
            </div>
        ...
        <div
            class="bg-white border border-[#E8EBED] rounded-[1.5rem] shadow-[0_2px_12px_rgb(0,0,0,0.02)] flex flex-col flex-1 overflow-hidden"
        >
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-[#E8EBED] bg-white">
                            <th
                                class="py-5 px-6 font-bold text-[10px] tracking-[0.15em] text-[#aab2bf] uppercase whitespace-nowrap min-w-[140px]"
                            >
                                ORDER ID
                            </th>
                            <th
                            ...
                            <th
                                class="py-5 px-6 font-bold text-[10px] tracking-[0.15em] text-[#aab2bf] uppercase whitespace-nowrap text-right w-[100px]"
                            >
                                ACTION
                            </th>
                        </tr>
                            ...
                            >
                                ...
                                <td class="px-4 py-[22px]">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-[34px] h-[34px] rounded-full bg-[#EAF2ED] text-[#2E5136] text-[11px] font-bold flex items-center justify-center shrink-0"
                                        >
                                            {{ strtoupper(substr($order->nama_pembeli, 0, 2)) }}
                                        </div>
                                        <div>
                                            <div
                                                class="text-[14px] text-[#1A1C19] font-medium"
                                            >
                                                {{ $order->nama_pembeli }}
                                            </div>
                                            <div
                                ...
                                <td class="px-4 py-[22px]">
                                    <div
                                        class="text-[13.5px] text-gray-500 font-medium"
                                    >
                                ...
                                <td class="px-4 py-[22px]">
                                    ...
                                    @if ($order->invoice)
                                        ...
                                        @endphp
                                        <div
                                            class="text-[11px] font-bold mt-0.5 {{ $payLabel[0] }}"
                                        >
                                            {{ $payLabel[1] }}
                                        </div>
                                    @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    >
                                        <div
                                            class="w-14 h-14 rounded-full bg-[#f9fafb] flex items-center justify-center text-3xl"
                                        >
                                            📦
                                        </div>
                                        <p class="text-gray-400 text-sm font-medium">
                                            @if (request('search') || request('status'))
                                                ...
                                                dengan filter.
                                            @else
                                                Belum ada pesanan masuk. Share
                                                link storefront kamu untuk mulai
                                                menerima pesanan.
                                            @endif
                                        </p>
                                        @if (request('search') || request('status'))
            ...
            @if ($orders->hasPages())
                <div
                    class="px-6 py-[22px] flex flex-col md:flex-row items-center justify-between gap-4 mt-auto border-t border-[#E8EBED]/60"
                >
        ...
        <div
            class="mt-8 mb-4 lg:mb-2 text-center text-[11px] font-medium text-gray-400"
        >
            © {{ date('Y') }} MyLinx
        </div>
    </div>
