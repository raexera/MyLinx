...
@else
    <section>
        <h2 class="text-2xl font-serif text-[var(--text-main)] mb-6">
            Produk Kami
        </h2>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                @foreach ($produks as $produk)
                    ...
                    >
                        @if ($produk->gambar)
                            <div
                                class="aspect-square overflow-hidden bg-[var(--input-bg)]"
                            >
                                <img
                        ...
                        @else
                            <div
                                class="aspect-square flex items-center justify-center bg-[var(--input-bg)] text-4xl text-[var(--text-muted)] opacity-50"
                            >
                                📦
                            </div>
                        @endif
                        <div class="p-3">
                            <h3
                                class="font-medium text-sm text-[var(--text-main)] line-clamp-2 hover:text-[var(--accent)] transition-colors"
                            >
                                {{ $produk->nama_produk }}
                            </h3>
            ...
            <div class="space-y-3">
                @foreach ($produks as $produk)
                    ...
                    >
                        @if ($produk->gambar)
                            <div
                                class="h-20 w-20 rounded-lg overflow-hidden bg-[var(--input-bg)] flex-shrink-0"
                            >
                                <img
                                    src="{{ asset('storage/' . $produk->gambar) }}"
                                    alt="{{ $produk->nama_produk }}"
                        ...
                        @else
                            <div
                                class="h-20 w-20 rounded-lg bg-[var(--input-bg)] flex items-center justify-center text-2xl text-[var(--text-muted)] opacity-50 flex-shrink-0"
                            >
                                📦
                            </div>
                        @endif
                        <div class="flex-1 min-w-0">
                            <h3
                                class="font-medium text-[var(--text-main)] hover:text-[var(--accent)] transition-colors"
                            >
                                {{ $produk->nama_produk }}
                            </h3>
                            ...
                            <div class="mt-2 flex items-center justify-between">
                                ...
                                <span
                                    class="text-[11px] text-[var(--text-muted)] font-medium bg-[var(--input-bg)] px-2 py-0.5 rounded"
