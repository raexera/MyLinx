...
>
    ...
    >
        <div class="mx-auto max-w-4xl px-6 py-6">
            <div class="flex items-center gap-4">
                ...
                @else
                    <div
                        class="flex h-14 w-14 items-center justify-center rounded-full btn-accent text-xl font-bold ring-2 ring-[var(--border-color)]"
                    >
                ...
                <div>
                    <a
                        ...
                        class="text-2xl font-bold text-[var(--text-main)] hover:text-[var(--accent)] transition-colors"
                    >
    ...
    <main class="mx-auto max-w-4xl px-6 py-10 flex-1 w-full">
        ...
        <div
            class="overflow-hidden rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] shadow-sm"
        >
            <div class="grid gap-0 md:grid-cols-2">
                <div
                    class="bg-[var(--input-bg)] border-b md:border-b-0 md:border-r border-[var(--border-color)]"
                >
                    @if ($produk->gambar)
                        <img
                    ...
                    @else
                        <div
                            class="flex h-80 items-center justify-center text-6xl text-[var(--text-muted)] opacity-50"
                        >
                            📦
                        </div>
                    @endif
                ...
                <div class="flex flex-col justify-between p-8">
                    <div>
                        ...
                        <div class="mt-4">
                            @if ($produk->stok > 0)
                                <span
                                    class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm font-bold text-[var(--accent)]"
                            ...
                            @else
                                <span
                                    class="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1 text-sm font-bold text-red-600 border border-red-100"
                                >
                                    Stok Habis
                                </span>
                            @endif
                    ...
                    <div class="mt-8 flex flex-col gap-3">
                        @if ($produk->stok > 0)
                            <a
                                href="{{ route('tenant.checkout', [$tenant, $produk]) }}"
    ...
    <footer
        class="border-t border-[var(--border-color)] bg-[var(--card-bg)] py-6 text-center text-sm text-[var(--text-muted)] mt-auto"
    >
        Dibuat dengan
        <a
            href="{{ route('landing') }}"
