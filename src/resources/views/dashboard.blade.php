<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-5 mt-2 lg:mt-0 w-full lg:pr-4 xl:pr-8 mb-2">
            <div class="flex flex-col">
                ...
                <h1 class="text-[2.5rem] sm:text-5xl lg:text-[3.25rem] font-serif text-[#1A1C19] tracking-tight leading-[1.1] mb-2.5">Dashboard</h1>
    ...
    @endif
    @if(session('error'))
        <div class="mt-4 rounded-2xl bg-red-50 border border-red-100 px-5 py-4 text-[13.5px] font-semibold text-red-700 flex items-center gap-3 mb-6">
            ...
            <div class="bg-white rounded-[2rem] shadow-[0_2px_10px_rgb(0,0,0,0.015)] border border-[#E8EBED] flex flex-col mb-8 overflow-hidden">
                ...
                <div class="hidden md:block w-full overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-white">
                                <th class="py-5 px-6 font-bold text-[10px] tracking-[0.15em] text-[#aab2bf] uppercase whitespace-nowrap min-w-[140px] pl-8">ORDER ID</th>
                                <th class="py-5 px-4 font-bold text-[10px] tracking-[0.15em] text-[#aab2bf] uppercase whitespace-nowrap min-w-[220px]">CUSTOMER</th>
                                ...
                                <th class="py-5 px-6 font-bold text-[10px] tracking-[0.15em] text-[#aab2bf] uppercase whitespace-nowrap text-right w-[100px] pr-8">AKSI</th>
                            </tr>
                        </thead>
                            ...
                            @foreach($recentOrders as $order)
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group cursor-pointer"
                                    ...
                                    <td class="px-6 py-[22px] pl-8">
                                        ...
                                        <div class="text-[11.5px] text-gray-400 mt-0.5">{{ $order->created_at->diffForHumans() }}</div>
                                    </td>
                                    <td class="px-4 py-[22px]">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <div class="text-[14px] text-[#1A1C19] font-medium leading-tight truncate">{{ $order->nama_pembeli }}</div>
                                                <div class="text-[11.5px] text-gray-400 truncate mt-0.5">{{ $order->email_pembeli }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-[22px]">
                                    ...
                                    <td class="px-6 py-[22px] text-right pr-8">
                ...
                <div class="md:hidden divide-y divide-[#E8EBED]/60">
                    @foreach($recentOrders as $order)
                        @php
                            ...
                            $sc = $statusMap[$order->status] ?? ['bg-gray-100 text-gray-500', '#9ca3af'];
                        @endphp
                        <a href="{{ route('order.show', $order) }}" class="block p-5 hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-start justify-between gap-3 mb-3">
                                <div class="min-w-0">
                                    <div class="font-bold text-[14px] text-[#1A1C19] truncate">{{ $order->nama_pembeli }}</div>
                                    <div class="text-[11.5px] text-gray-400 font-mono">{{ $order->kode_order }}</div>
                                </div>
                                <span class="{{ $sc[0] }} text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1.5 shrink-0">
                            ...
                            <div class="flex items-center justify-between pt-3 border-t border-[#F0F2F3]">
                                ...
                                <span class="text-[11.5px] text-gray-400">{{ $order->created_at->diffForHumans() }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
